...
def greed_steps(goal, n=8):
    cells = [(0, [], [])]
    while cells:
        row, state, path = cells.pop()
        if row == n:
            if tuple(state) == goal:
        ...
        for col in range(n):
            if is_safe(state, row, col):
                new_state = state + [col]
                new_cost = abs(goal[row] - col)
                new_path = path + [(row, col, new_cost)]
                yield new_state, False, new_path
                c.append((row + 1, new_state, new_path))
        cells = func_h(cells, c)

def main():
    root = tk.Tk()
    root.title("N-Queens Greedy")
    ...
    def run():
        ...
        if not sol:
            return
        goal_holder["gen"] = greed_steps(sol, 8)
        step()
    ...
    def export_file():
        ...
        with open("nqueens_result.txt", "w", encoding="utf-8") as f:
            f.write("=== LỜI GIẢI N-QUEENS (THUẬT TOÁN THAM LAM) ===\n\n")
            for i, (r, c, cost) in enumerate(path):
                f.write(f"Hậu thứ {i+1}: hàng {r+1}, cột {c+1}, chi phí {cost}\n")
